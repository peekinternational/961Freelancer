...
@section('title') Edit Language @endsection
@section('css')
...
<div class="row">
  <div class="col-lg-12">
    <div class="card">
      <div class="card-body">
        <h4 class="card-title mb-4">Edit Language</h4>
        <form id="update-language-form" enctype="multipart/form-data">
          @csrf
          <div class="form-group row mb-4">
            <label for="projectname" class="col-form-label col-lg-2">Language Name</label>
            <div class="col-lg-10">
              <input id="languagename" name="language_name" value="{{$getSingleData->language_name}}" type="text" class="form-control" placeholder="Enter Language Name...">
            </div>
          </div>
        </form>
        <div class="row justify-content-end">
          <div class="col-lg-10">
            <button type="submit" form="update-language-form" class="btn btn-primary">Edit Language</button>
          </div>
        </div>
      </div>
    </div>
...
<script>
  ...
  $('#update-language-form').on('submit', function(event){
      event.preventDefault();
      let createFormDatas = $('#update-language-form').serialize();
    $.ajax({
        url: "{{url('admin/languages/'.$getSingleData->id)}}",
        type: 'PATCH',
